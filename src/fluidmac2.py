...
######   Using delta Interpolation (Poisseuille Benchmark shows that this skew is giving problems)
#     def skew(self, u):
...
####  Equivalent code using sk
    def skew(self, u):
        ip, im = self.ip, self.im
        w=u*1.               
        ux_y = (u[0][:,im]+u[0])/2   ## u_x on gridpoints of u_y
        ux_y = (ux_y[ip,:]+ux_y)/2
        uy_x = (u[1][im,:]+u[1])/2   ## u_y on gridpoints of u_x
        uy_x = (uy_x+uy_x[:,ip])/2
        w[0]=self.sk([u[0], uy_x], u[0]) ## On grid x, u_x is given and u_y is interpolated
        w[1]=self.sk([ux_y, u[1]], u[1])
        return w
